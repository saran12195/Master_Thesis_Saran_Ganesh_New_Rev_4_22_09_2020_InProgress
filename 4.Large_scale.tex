...
%\centering
%\hspace*{-1.2cm}
    \includegraphics[height = 8.5cm,width = \textwidth]{Diagrams/Chapter_4/Steps_for_Scaling_2.pdf}
    \caption{OWF-1 and OWF-2 connected in parallel to external AC system}
    \label{fig:Steps_for_Scaling_2}
\end{figure}

    \item In the third step, the external \gls{AC} system is replaced by an offshore converter station consisting of the average \gls{EMT} model of \gls{MMC} (\gls{MMC}-1) and the interface transformer (IT-1) available in the CIGRE B4 DC Grid Test System \cite{vrana2013cigre}. The same is depicted in Figure \ref{fig:Steps_for_Scaling_3}. As the final layout network would be extensive, it is required to split the network into subsystems, as mentioned in Section  \ref{split_system}. The splitting into two subsystems is performed in this step. The offshore converter station is modelled in subsystem-1, and the \gls{OWF}s are modelled in subsystem-2. \gls{MMC}-1 is designed to operate in V/F control (or grid forming control) which is explained at the end of this chapter.

\begin{figure}[H]
%\centering
%\hspace*{-1.2cm}
    \includegraphics[height = 7.5cm,width = \textwidth]{Diagrams/Chapter_4/Steps_for_Scaling_3.pdf}
    \caption{OWF-1 and OWF-2 connected in parallel to MMC-1}
    \label{fig:Steps_for_Scaling_3}
\end{figure}
...
The average model of Type-4 \gls{WG} available in RSCAD is used for this work as well. The aggregated \gls{OWF} model represented in a small time step environment described in Section \ref{OWF} is used here for all the four \gls{OWF}s. As the entire network is extensive, it is split into two subsystems in RSCAD, as shown in Figure \ref{fig:WT4_MMC2} by employing the technique explained in Section \ref{split_system}. The four \gls{OWF}s are modelled in subsystem-2, and the rest of the system is modelled in subsystem-1. Each subsystem requires one rack for operation, and hence two NovaCor racks are used for the network simulation. It is also possible to simulate the network with PB5 processor. However, this requires a total of three subsystems since PB5 racks allow only small network solutions to be solved in one rack \cite{noauthor_pb5_nodate_1}. Therefore, two \gls{OWF}s are to be modelled in subsystem-3, other two \gls{OWF}s in subsystem-2 and the rest of the network in subsystem-1. Henceforth, the simulations are considered only in NovaCor processor for this study.  
...
\subsection{Offshore Converter Stations}\label{offshore_conv_stations}
...
\subfloat[Symmetrical monopole configuration \cite{sharifabadi2016design}]{%
  \includegraphics[clip,width=0.6\columnwidth]{Diagrams/Chapter_4/Monopole_rep.pdf}%
\label{Monopole_rep}}
...
\subfloat[Symmetrical monopole configuration by equivalent representation of DC side of onshore MMC station using a constant DC source]{%
  \includegraphics[clip,width=0.6\columnwidth]{Diagrams/Chapter_4/Monopole_rep_2.pdf}%
...
\subfloat[Outer control loop representation for d-axis ($i_{d\_ref\_2}$)]{%
  \includegraphics[height = 7cm,width = 10cm]{Diagrams/Chapter_4/Control_loop_MMC_Id.pdf}%
\label{Control_loop_MMC_Id}}

\vspace{5mm}
%\hspace{4cm}
\subfloat[Outer control loop representation for q-axis ($i_{q\_ref\_2}$)]{%
  \includegraphics[height = 7cm,width = 10cm]{Diagrams/Chapter_4/Control_loop_MMC_Iq.pdf}%
...
where $R_{trafo\_2}$ is the interface transformer (IT-2b) resistance, $R_{arm\_2}$ is the arm resistance in \gls{MMC}-2, $v_{PCC_j}$ is the voltage at \gls{HV} bus of IT-2a, $j$ represents phases $a$, $b$ and $c$. Upper arm and lower arm of \gls{MMC}-2 are denoted by $u$ and $l$ respectively.
